...
import {
  ...
  Shield, Sliders, Bell, Crown, Key, AlertCircle, Phone, Mail
} from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { useTheme } from '../../context/ThemeContext';
import { supabase } from '../../lib/supabase';

type Tab = 'identity' | 'contact' | 'security' | 'preferences';
...
const SuperAdminProfile = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
      ...
      {/* ── Hero Banner ── */}
      <div style={{ background: `linear-gradient(135deg, ${d.navy} 0%, #263060 100%)`, borderRadius: 18, padding: 'clamp(1.25rem,3vw,1.75rem) clamp(1.25rem,3vw,2rem)', marginBottom: '1.25rem', boxShadow: '0 14px 44px rgba(28,35,64,0.28)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '-40%', right: '5%', width: 200, height: 200, background: d.gold, filter: 'blur(100px)', opacity: 0.1, borderRadius: '50%', pointerEvents: 'none' }} />
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
          <div style={{ width: 64, height: 64, borderRadius: '50%', background: `linear-gradient(135deg, ${d.gold} 0%, #b38d2a 100%)`, color: d.navy, fontWeight: 800, fontSize: '1.625rem', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 18px rgba(201,160,51,0.4)', flexShrink: 0, fontFamily: 'Outfit' }}>
            {(profile.first_name || user?.name || 'S').charAt(0).toUpperCase()}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.25rem', flexWrap: 'wrap' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.6875rem', color: d.gold, fontWeight: 700 }}>
                <Crown size={12} /> Super Administrator · Portal Command
              </span>
              <span style={{ fontSize: '0.5625rem', color: '#4ade80', fontWeight: 700 }}>✓ Full System Access</span>
            </div>
          </div>
        </div>
      </div>
        ...
        {activeTab === 'identity' && (
          <motion.div key="identity" initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -14 }} transition={{ duration: 0.25 }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.125rem', paddingBottom: '0.75rem', borderBottom: `1px solid ${d.border}` }}>
              ...
              <div>
                <h2 style={{ margin: 0, fontSize: '0.9375rem', fontWeight: 800, fontFamily: 'Outfit', color: d.text }}>Personal Identity & Demographics</h2>
                <p style={{ margin: 0, fontSize: '0.625rem', color: d.muted }}>Update your name, ID number, and personal details</p>
              </div>
            </div>
            <form onSubmit={saveIdentity} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.875rem' }}>
                {[
                  { label: 'First Name', key: 'first_name', type: 'text', req: true },
                  { label: 'Last Name', key: 'last_name', type: 'text', req: true },
                  { label: 'SA ID Number (13-digit)', key: 'sa_id_number', type: 'text', placeholder: '8501015000088' },
                  { label: 'Passport Number', key: 'passport_number', type: 'text', placeholder: 'A0000000' },
                  { label: 'Date of Birth', key: 'date_of_birth', type: 'date' },
                ].map(f => (
                  <div key={f.key} style={fieldStyle}>
                    <label style={labelStyle}>{f.label}</label>
                    <input type={f.type} required={f.req} placeholder={f.placeholder} value={profile[f.key] || ''} onChange={e => setProfile({ ...profile, [f.key]: e.target.value })} style={inputStyle}
                      onFocus={e => e.currentTarget.style.borderColor = d.gold + '55'}
                      onBlur={e => e.currentTarget.style.borderColor = isDark ? 'rgba(255,255,255,0.09)' : '#e2e8f0'} />
                  </div>
                ))}
                <div style={fieldStyle}>
                  <label style={labelStyle}>Gender</label>
                  <select value={profile.gender || 'male'} onChange={e => setProfile({ ...profile, gender: e.target.value })} style={inputStyle}>
                    {['male', 'female', 'other'].map(g => <option key={g} value={g} style={{ background: isDark ? '#1c2340' : '#fff' }}>{g.charAt(0).toUpperCase() + g.slice(1)}</option>)}
                  </select>
                </div>
              </div>
        ...
        {activeTab === 'contact' && (
          <motion.div key="contact" initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -14 }} transition={{ duration: 0.25 }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.125rem', paddingBottom: '0.75rem', borderBottom: `1px solid ${d.border}` }}>
              ...
              <div>
                <h2 style={{ margin: 0, fontSize: '0.9375rem', fontWeight: 800, fontFamily: 'Outfit', color: d.text }}>Contact & Physical Address</h2>
                <p style={{ margin: 0, fontSize: '0.625rem', color: d.muted }}>Update mobile number and administrative office address</p>
              </div>
            </div>
            <form onSubmit={saveContact} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.875rem' }}>
                <div style={fieldStyle}>
                  <label style={labelStyle}><Phone size={10} style={{ display: 'inline', marginRight: 4 }} />Mobile Phone</label>
                  <input type="tel" value={profile.phone || ''} onChange={e => setProfile({ ...profile, phone: e.target.value })} style={inputStyle}
                    onFocus={e => e.currentTarget.style.borderColor = d.gold + '55'}
                    onBlur={e => e.currentTarget.style.borderColor = isDark ? 'rgba(255,255,255,0.09)' : '#e2e8f0'} />
                </div>
                <div style={fieldStyle}>
                  <label style={labelStyle}><Mail size={10} style={{ display: 'inline', marginRight: 4 }} />Email (Read-only)</label>
                  <input type="email" disabled value={profile.email} style={{ ...inputStyle, background: isDark ? 'rgba(255,255,255,0.02)' : '#e2e8f0', color: d.muted, cursor: 'not-allowed' }} />
                </div>
                <div style={{ ...fieldStyle, gridColumn: 'span 2' }}>
                  <label style={labelStyle}>Address Line 1</label>
                  <input type="text" placeholder="123 Executive Park" value={profile.address_line1 || ''} onChange={e => setProfile({ ...profile, address_line1: e.target.value })} style={inputStyle}
                    onFocus={e => e.currentTarget.style.borderColor = d.gold + '55'}
                    onBlur={e => e.currentTarget.style.borderColor = isDark ? 'rgba(255,255,255,0.09)' : '#e2e8f0'} />
                </div>
                {[
                  { label: 'Suburb', key: 'suburb', placeholder: 'Sandton' },
                  { label: 'City', key: 'city', placeholder: 'Johannesburg' },
                  { label: 'Province', key: 'province', placeholder: 'Gauteng' },
                  { label: 'Postal Code', key: 'postal_code', placeholder: '2196' },
                ].map(f => (
                  <div key={f.key} style={fieldStyle}>
                    <label style={labelStyle}>{f.label}</label>
                    <input type="text" placeholder={f.placeholder} value={profile[f.key] || ''} onChange={e => setProfile({ ...profile, [f.key]: e.target.value })} style={inputStyle}
                      onFocus={e => e.currentTarget.style.borderColor = d.gold + '55'}
                      onBlur={e => e.currentTarget.style.borderColor = isDark ? 'rgba(255,255,255,0.09)' : '#e2e8f0'} />
                  </div>
                ))}
        ...
        {activeTab === 'security' && (
          <motion.div key="security" initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -14 }} transition={{ duration: 0.25 }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.125rem', paddingBottom: '0.75rem', borderBottom: `1px solid ${d.border}` }}>
              ...
              <div>
                <h2 style={{ margin: 0, fontSize: '0.9375rem', fontWeight: 800, fontFamily: 'Outfit', color: d.text }}>Security & Credentials</h2>
                <p style={{ margin: 0, fontSize: '0.625rem', color: d.muted }}>Manage authentication password and review system access</p>
              </div>
            </div>
            {/* Role badge */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.875rem 1rem', borderRadius: 10, background: isDark ? 'rgba(201,160,51,0.08)' : 'rgba(201,160,51,0.06)', border: `1px solid ${isDark ? 'rgba(201,160,51,0.2)' : 'rgba(201,160,51,0.15)'}`, marginBottom: '1.25rem' }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: d.navy, display: 'flex', alignItems: 'center', justifyContent: 'center', color: d.gold, flexShrink: 0 }}><Crown size={18} /></div>
              <div>
                <div style={{ fontSize: '0.8125rem', fontWeight: 800, color: d.text }}>Super Administrator — Full Access</div>
                <div style={{ fontSize: '0.5625rem', color: d.muted }}>Multi-clinic management · Financial reconciliation · User administration · Risk oversight</div>
              </div>
            </div>
            <form onSubmit={saveSecurity} style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem', maxWidth: 420 }}>
              <div style={fieldStyle}>
                <label style={labelStyle}><Key size={10} style={{ display: 'inline', marginRight: 4 }} />New Password (min 8 characters)</label>
        ...
        {activeTab === 'preferences' && (
          <motion.div key="preferences" initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -14 }} transition={{ duration: 0.25 }}
            ...
            <div style={{ background: d.card, borderRadius: 14, border: `1px solid ${d.border}`, padding: 'clamp(1rem,3vw,1.5rem)', boxShadow: d.shad }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', paddingBottom: '0.75rem', borderBottom: `1px solid ${d.border}` }}>
                <Sliders size={16} color={d.gold} />
                <h2 style={{ margin: 0, fontSize: '0.9375rem', fontWeight: 800, fontFamily: 'Outfit', color: d.text }}>Theme & Appearance</h2>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                {[
                  { label: 'Light Mode', val: 'light', icon: <Sun size={24} /> },
                  { label: 'Dark Executive Mode', val: 'dark', icon: <Moon size={24} /> },
                ].map(t => (
                  <button key={t.val} onClick={() => theme !== t.val && toggleTheme()}
                    style={{ padding: '1.125rem', borderRadius: 11, border: `2px solid ${theme === t.val ? d.gold : d.border}`, background: theme === t.val ? d.goldS : d.surf, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', transition: 'all 0.2s' }}>
                    ...
                    {theme === t.val && <span style={{ fontSize: '0.5rem', fontWeight: 800, color: d.gold, letterSpacing: '0.05em' }}>ACTIVE</span>}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={{ background: d.card, borderRadius: 14, border: `1px solid ${d.border}`, padding: 'clamp(1rem,3vw,1.5rem)', boxShadow: d.shad }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', paddingBottom: '0.75rem', borderBottom: `1px solid ${d.border}` }}>
                <Bell size={16} color={d.gold} />
                <h2 style={{ margin: 0, fontSize: '0.9375rem', fontWeight: 800, fontFamily: 'Outfit', color: d.text }}>Notification Preferences</h2>
              </div>
              {[
                { label: 'Email Notifications', sub: 'Daily summaries and system alerts via email', val: emailAlerts, set: setEmailAlerts },
                { label: 'SMS Alerts', sub: 'Critical network alerts sent by SMS', val: smsAlerts, set: setSmsAlerts },
              ].map((item, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.875rem 0', borderBottom: i === 0 ? `1px solid ${d.border}` : 'none' }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: '0.8125rem', color: d.text }}>{item.label}</div>
                    <div style={{ fontSize: '0.5625rem', color: d.muted, marginTop: 2 }}>{item.sub}</div>
                  </div>
                  <div onClick={() => item.set(!item.val)} style={{ width: 42, height: 23, background: item.val ? d.gold : (isDark ? 'rgba(255,255,255,0.12)' : '#cbd5e1'), borderRadius: 12, position: 'relative', cursor: 'pointer', transition: 'background 0.25s', flexShrink: 0 }}>
                    <motion.div animate={{ left: item.val ? '21px' : '2px' }} transition={{ type: 'spring', stiffness: 350, damping: 28 }}
                      style={{ width: 19, height: 19, background: '#fff', borderRadius: '50%', position: 'absolute', top: 2, boxShadow: '0 1px 4px rgba(0,0,0,0.18)' }} />
                  </div>
                </div>
              ))}
            </div>
