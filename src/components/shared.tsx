...
export const QRCanvas = ({ value, size = 72 }: QRCanvasProps) => {
...
export const MemberCard = ({ compact, memberNum, memberName, planName, status, tokensRemaining, totalTokens }: MemberCardProps) => {
  const hasTokenData = tokensRemaining !== undefined && totalTokens !== undefined;
  const rem = tokensRemaining ?? 0;
  const tot = totalTokens ?? 0;
  const isUnlimited = tot === -1;

  const tokenColor = !hasTokenData ? "#8DA0C2" : isUnlimited || rem > 1 ? "#2DD4BF" : rem === 1 ? "#FBBF24" : "#F87171";
  const tokenBg = !hasTokenData ? "rgba(141,160,194,0.15)" : isUnlimited || rem > 1 ? "rgba(19, 168, 158, 0.25)" : rem === 1 ? "rgba(201, 150, 58, 0.25)" : "rgba(209, 67, 67, 0.3)";
  const tokenBorder = !hasTokenData ? "rgba(141,160,194,0.3)" : isUnlimited || rem > 1 ? "rgba(19, 168, 158, 0.5)" : rem === 1 ? "rgba(201, 150, 58, 0.5)" : "rgba(209, 67, 67, 0.6)";
  const tokenText = !hasTokenData ? "LOADING TOKENS..." : isUnlimited ? "UNLIMITED" : `${rem} / ${tot} REMAINING`;
  ...
  return (
    <div style={{
      ...
      <div style={{ marginTop:20, position:"relative", zIndex:1, display:"flex", justifyContent:"space-between" }}>
        ...
        <div style={{ background:"#fff", borderRadius:10, width:84, height:84, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, marginLeft: 12, padding: 4 }}>
          {memberNum ? (
            <QRCanvas value={memberNum} size={76} />
          ) : (
            <div style={{ width: 76, height: 76, background: '#E5E9F0', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: '#8DA0C2', textAlign: 'center' }}>No Card</div>
          )}
        </div>
      </div>
